...
public class EventServiceImpl extends RemoteServiceServlet implements
	...
	public void addEvent(Event event) throws Exception {
		...
		if (!(event instanceof BirthdayEvent)) {
			throw new ServiceException(
					"only birthday events are supported");
		}
		BirthdayEvent be = (BirthdayEvent) event;
		// validate if all the fields are present
		if (be.getDate() == null) {
			throw new ServiceException("Event date should not be null");
		}
		if (be.getOwner() == null) {
			throw new ServiceException("Event ownder should not be null");
		}
		if (be.getBirthdayPerson() == null) {
			throw new ServiceException("Birthday person should not be null");
		}
		...
		if (ev.exitsEventByBirthdayPerson(be.getBirthdayPerson())) {
			throw new ServiceException("Birthday person already has a birthday");
		}
		// save it using the EventDAO
		try {
			ev.addEvent((BirthdayEvent) event);
		} catch (RuntimeException r) {
			if (r instanceof EventExistException) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.cmu.tsp6.client.EventService#removeEvent(java.lang.String,
	 * java.lang.String)
	 */
	@Override
	...
	public void updateEvent(Event event) throws Exception {
		...
		if (ev.exitsEventByBirthdayPerson(be.getBirthdayPerson())) {
			throw new IllegalStateException(
					"Birthday person already has a birthday");
		}
		// call edit on the dao
	...
	public User getCurrentUser() {
		User u = (User) this.getThreadLocalRequest().getSession().getAttribute(
				WebCalendarConstants.USER);
		return u;
	}
